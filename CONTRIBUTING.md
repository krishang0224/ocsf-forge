...
Use Python 3.11 and install `requirements-dev.txt` in a virtual environment. Before opening a pull request, run `ruff check .`, `pytest -q`, and `docker compose --profile streaming --profile operations config --quiet`. These checks do not require a running lakehouse.
